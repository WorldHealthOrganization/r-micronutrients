...
random_datset <- function(n) {
  data.frame(
    ...
    ferritin_measurement = pmax(0, rnorm(n, mean = 30, sd = 25)),
    crp_measurement = pmax(0, rnorm(n, mean = 2, sd = 10)),
    agp_measurement = pmax(0, rnorm(n, mean = 0.5, sd = 0.5)),
    haemoglobin_measurement = pmax(0, rnorm(n, mean = 100, sd = 50)),
